...
module.exports = class extends think.Model {
    ...
    getLatestOrderExpress(orderId) {
        ...
        return _asyncToGenerator(function* () {
            // 这个是快递鸟的接口，查不了顺丰。顺丰用阿里云的付费接口，在order.js中已经写好
            ...
            const orderExpress = yield _this.where({
                order_id: orderId
            }).find();
     ...
     * @returns {Promise.<*>}
     */
    getLatestOrderExpressByAli(orderId) {
        var _this2 = this;
        ...
        return _asyncToGenerator(function* () {
            ...
            let info = yield _this2.model('order_express').where({
                order_id: orderId
            }).find();
            if (think.isEmpty(info)) {
                return _this2.fail(400, '暂无物流信息');
            }
            const expressInfo = yield _this2.model('order_express').where({
                order_id: orderId
            }).find();
            ...
            if (is_finish == 1) {
                ...
                let lastExpressInfo = yield _this2.getExpressInfo(shipperName, expressNo);
                ...
                deliverystatus = yield _this2.getDeliverystatus(deliverystatus);
                ...
                yield _this2.model('order_express').where({
                    order_id: orderId
                }).update(dataInfo);
                let express = yield _this2.model('order_express').where({
                    order_id: orderId
                }).find();
                return express;
            }
    ...
    getMianExpress(orderId, senderInfo, receiverInfo, expressType) {
        var _this3 = this;
        ...
        return _asyncToGenerator(function* () {
            ...
            if (testSwitch == 1) {
                ...
                if (expressType < 4) {
                    if (expressType == 1 || expressType == 2) {
                        let info = yield _this3.model('shipper').where({
                            name: '顺丰速运'
                        }).find();
            ...
            const orderExpress = yield _this3.model('order').where({
                id: orderId
            }).find();
            if (think.isEmpty(orderExpress)) {
                let error = 400;
            ...
            const ExpressSerivce = think.service('express', 'api');
            const latestExpressInfo = yield ExpressSerivce.mianExpress(returnExpressInfo);
            // 将保价金额和时间回传。
            const currentTime = parseInt(new Date().getTime() / 1000);
    ...
    printExpress() {
        return _asyncToGenerator(function* () {
            const ExpressSerivce = think.service('express', 'api');
            const latestExpressInfo = yield ExpressSerivce.buildForm();
            return latestExpressInfo;
        })();
    }
};
